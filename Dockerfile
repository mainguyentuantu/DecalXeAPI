...
# 5. Bước "Nướng bánh": Xuất bản ứng dụng, chỉ định rõ file dự án cần publish.
...
# 6. Bước "Đặt bánh lên đĩa": Sử dụng một image runtime nhẹ hơn, chỉ chứa những gì cần thiết để chạy.
FROM mcr.microsoft.com/dotnet/aspnet:8.0 AS final
WORKDIR /app

# 7. Bước "Chuyển bánh từ lò ra đĩa": Sao chép kết quả đã publish ở giai đoạn build vào đây.
COPY --from=build /app/publish .

# 8. Bước "Mở cửa tiệm và nói chào khách": Cấu hình cổng mà ứng dụng sẽ lắng nghe.
# Railway/OnRender sẽ sử dụng cổng này.
ENV ASPNETCORE_URLS=http://+:8080
ENV ASPNETCORE_ENVIRONMENT=Production
EXPOSE 8080

# 9. Bước "Bắt đầu làm việc": Lệnh khởi động ứng dụng.
