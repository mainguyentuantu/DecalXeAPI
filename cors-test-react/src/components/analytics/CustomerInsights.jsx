...
const CustomerInsights = () => {
  ...
  const filteredCustomers = React.useMemo(() => {
    ...
    let filtered = customerData.topCustomers.filter(customer => {
      ...
      if (selectedSegment !== 'all') {
        ...
        matchesSegment = segment.toLowerCase() === selectedSegment.toLowerCase();
      }
  ...
  const handleExportPDF = async () => {
    try {
      await exportUtils.exportToPDF('customer-insights-container', 'customer-insights.pdf');
  ...
  const handleExportExcel = () => {
    try {
      ...
      exportUtils.exportToExcel(exportData, 'customer-insights.xlsx', 'Phân tích khách hàng');
    ...
    <div className="space-y-6">
      ...
      <div className="flex flex-col sm:flex-row justify-between items-start sm:items-center gap-4">
        <div>
          ...
          <p className="text-gray-600 mt-1">Hiểu rõ hành vi và giá trị khách hàng</p>
        </div>
        ...
        <div className="grid grid-cols-1 md:grid-cols-4 gap-6">
          <div className="bg-white p-6 rounded-lg shadow-sm border">
            ...
            <div className="mt-4 flex items-center">
              ...
              <span className="text-sm text-green-600 ml-1">+5.2% tháng này</span>
            </div>
          </div>
          ...
          <div className="bg-white p-6 rounded-lg shadow-sm border">
            <div className="flex items-center justify-between">
              <div>
                <p className="text-sm font-medium text-gray-600">KH hoạt động</p>
                <p className="text-2xl font-bold text-gray-900">
            ...
            <div className="mt-4 flex items-center">
              ...
              <span className="text-sm text-green-600 ml-1">
                {customerData?.totalCustomers > 0 ? 
                  exportUtils.formatPercentage((customerData.activeCustomers / customerData.totalCustomers) * 100) : 
                  '0%'
                } tỷ lệ hoạt động
              </span>
            </div>
          ...
          <div className="bg-white p-6 rounded-lg shadow-sm border">
            <div className="flex items-center justify-between">
              <div>
                <p className="text-sm font-medium text-gray-600">Giá trị KH TB</p>
                <p className="text-2xl font-bold text-gray-900">
                  {exportUtils.formatCurrency(customerData?.averageCustomerValue || 0)}
                </p>
              </div>
            ...
            <div className="mt-4 flex items-center">
              <TrendingUp className="w-4 h-4 text-green-500" />
              <span className="text-sm text-green-600 ml-1">+8.7% so với tháng trước</span>
            </div>
          </div>
          ...
          <div className="bg-white p-6 rounded-lg shadow-sm border">
              ...
              <div>
                ...
                <p className="text-2xl font-bold text-gray-900">
                  {exportUtils.formatCurrency(
                    customerData?.topCustomers?.slice(0, 10).reduce((sum, c) => sum + c.totalSpent, 0) || 0
                  )}
                </p>
              </div>
            ...
            <div className="mt-4 flex items-center">
              ...
              <span className="text-sm text-yellow-600 ml-1">Khách hàng VIP</span>
            </div>
          </div>
        </div>
              ...
              <PieChart>
                <Pie
                  ...
                  label={({ segment, count, totalValue }) => 
                    `${segment}: ${count} (${exportUtils.formatCurrency(totalValue)})`
                  }
                  outerRadius={80}
                  fill="#8884d8"
                  dataKey="count"
                ...
                <Tooltip 
                  formatter={(value, name, props) => [
                    `${value} khách hàng (${exportUtils.formatCurrency(props.payload.totalValue)})`,
                    'Số lượng'
                  ]}
                />
                <Legend />
          ...
          <div className="bg-white p-6 rounded-lg shadow-sm border">
            ...
            <ResponsiveContainer width="100%" height={300}>
              <BarChart data={customerData?.customerSegments || []}>
                ...
                <Tooltip 
                  formatter={(value) => [exportUtils.formatCurrency(value), 'Giá trị trung bình']}
                />
            ...
            {customerData?.topCustomers?.slice(0, 6).map((customer, index) => (
              <div key={customer.customerID} className="border border-gray-200 rounded-lg p-4 hover:shadow-md transition-shadow">
                <div className="flex items-start justify-between mb-3">
                  ...
                  <span className={`inline-flex px-2 py-1 text-xs font-semibold rounded-full ${getSegmentColor(getCustomerSegment(customer.totalSpent))}`}>
        ...
        <div className="bg-white p-6 rounded-lg shadow-sm border">
          ...
          <div className="grid grid-cols-1 md:grid-cols-3 gap-6">
            <div className="text-center">
              ...
              <h4 className="font-medium text-gray-900 mb-2">Khách hàng trung thành</h4>
              <p className="text-2xl font-bold text-green-600 mb-1">
                {customerData?.topCustomers?.filter(c => c.totalOrders >= 5).length || 0}
              </p>
            ...
            <div className="text-center">
              ...
              <h4 className="font-medium text-gray-900 mb-2">Khách hàng thường xuyên</h4>
              <p className="text-2xl font-bold text-blue-600 mb-1">
                {customerData?.topCustomers?.filter(c => c.totalOrders >= 2 && c.totalOrders < 5).length || 0}
              </p>
            ...
            <div className="text-center">
              ...
              <p className="text-2xl font-bold text-yellow-600 mb-1">
                {customerData?.topCustomers?.filter(c => c.totalOrders === 1).length || 0}
              </p>
