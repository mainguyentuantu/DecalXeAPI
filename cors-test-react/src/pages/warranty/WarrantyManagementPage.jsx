...
const WarrantyManagementPage = () => {
  ...
  const handleSubmit = async (e) => {
    ...
    try {
      if (selectedWarranty) {
        await warrantyService.updateWarranty(selectedWarranty.warrantyID, formData);
      } else {
  ...
  const filteredWarranties = warranties.filter(warranty => {
    ...
    const matchesStatus = filterStatus === 'all' || warranty.warrantyStatus === filterStatus;
    
    return matchesSearch && matchesStatus;
  });

  const activeWarranties = warranties.filter(w => w.warrantyStatus === 'active').length;
  const expiredWarranties = warranties.filter(w => w.warrantyStatus === 'expired').length;
  const cancelledWarranties = warranties.filter(w => w.warrantyStatus === 'cancelled').length;
    ...
    <div className="min-h-screen bg-gray-50 py-8">
      <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8">
        ...
        <div className="mb-8">
          ...
          <p className="mt-2 text-gray-600">Theo dõi và quản lý các chính sách bảo hành</p>
        </div>
        ...
        <div className="grid grid-cols-1 md:grid-cols-4 gap-6 mb-8">
          <div className="bg-white rounded-lg shadow p-6">
            <div className="flex items-center">
              ...
              <div className="ml-4">
                <p className="text-sm font-medium text-gray-600">Tổng Bảo Hành</p>
        ...
        <div className="bg-white shadow rounded-lg overflow-hidden">
          <div className="px-6 py-4 border-b border-gray-200">
            <h3 className="text-lg font-medium text-gray-900">Danh Sách Bảo Hành</h3>
          </div>
                ...
                {filteredWarranties.map((warranty) => (
                  <tr key={warranty.warrantyID} className="hover:bg-gray-50">
                    ...
                    <td className="px-6 py-4 whitespace-nowrap">
                      <span className={`inline-flex px-2 py-1 text-xs font-semibold rounded-full ${getTypeColor(warranty.warrantyType)}`}>
                        {getTypeText(warranty.warrantyType)}
                      </span>
                    </td>
                    <td className="px-6 py-4 whitespace-nowrap">
                      <span className={`inline-flex px-2 py-1 text-xs font-semibold rounded-full ${getStatusColor(warranty.warrantyStatus)}`}>
                    ...
                    <td className="px-6 py-4 whitespace-nowrap text-sm text-gray-900">
                      <span className={isWarrantyExpired(warranty.endDate) ? 'text-red-600 font-medium' : ''}>
            ...
            <div className="relative top-20 mx-auto p-5 border w-96 shadow-lg rounded-md bg-white">
              <div className="mt-3">
                ...
                <form onSubmit={handleSubmit} className="space-y-4">
                  <div>
                    <label className="block text-sm font-medium text-gray-700">Mã Xe</label>
                    <input
                      type="text"
