  ...
  return (
    <div className="p-8 max-w-4xl mx-auto">
      ...
      <form onSubmit={handleSubmit} className="space-y-8">
            ...
            <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
              ...
              >
                ...
                {formData?.vehicleModels
                  ?.filter(model => !formState.vehicleBrand || model.vehicleBrandId === formState.vehicleBrand)
                  ?.map(model => (
        ...
        <Card>
          <div className="p-6">
            <h2 className="text-xl font-semibold mb-4">Dịch vụ và loại decal</h2>
            ...
            <div className="mb-6">
              ...
              <div className="grid grid-cols-1 md:grid-cols-2 gap-3">
                {formData?.decalServices?.map(service => (
                  <label key={service.serviceId} className="flex items-center space-x-3">
                    <input
                      type="checkbox"
                      checked={formState.decalServices.includes(service.serviceId)}
                      onChange={(e) => handleServiceChange(service.serviceId, e.target.checked)}
            ...
            <div>
              ...
              <div className="grid grid-cols-1 md:grid-cols-2 gap-3">
                {formData?.decalTypes?.map(type => (
                  <label key={type.typeId} className="flex items-center space-x-3">
                    <input
                      type="checkbox"
                      checked={formState.decalTypes.includes(type.typeId)}
                      onChange={(e) => handleTypeChange(type.typeId, e.target.checked)}
                      className="rounded border-gray-300"
                    />
