...
const OrderTrackingPage = () => {
  ...
  const { data: trackingData, isLoading, error, refetch } = useOrderTracking(
  ...
  const getStageColor = (stage) => {
    switch (stage?.toLowerCase()) {
      case 'new profile': return 'bg-blue-100 text-blue-800';
  ...
  return (
    <div className="p-8 max-w-6xl mx-auto">
      <div className="mb-8">
        ...
        <p className="text-gray-600">Tìm kiếm và theo dõi trạng thái đơn hàng</p>
      </div>
      ...
      {error && (
        <Card>
          <div className="p-6 text-center">
            <h3 className="text-lg font-semibold text-red-600 mb-2">Lỗi tìm kiếm</h3>
      ...
      {trackingData && trackingData.length === 0 && !isLoading && (
        <Card>
          <div className="p-6 text-center">
            ...
            <p className="text-gray-500">Vui lòng kiểm tra lại thông tin tìm kiếm</p>
          </div>
        </Card>
      )}
              ...
              <div className="p-6">
                ...
                <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-6 mb-6">
                  ...
                  <div>
                    ...
                    <div className="space-y-1 text-sm">
                      <p><span className="font-medium">Biển số:</span> {order.licensePlate || 'N/A'}</p>
                      <p><span className="font-medium">Hãng:</span> {order.vehicleBrand || 'N/A'}</p>
                      <p><span className="font-medium">Dòng:</span> {order.vehicleModel || 'N/A'}</p>
                    </div>
                  </div>
                  ...
                  <div>
                    ...
                    <div className="space-y-1 text-sm">
                      <p><span className="font-medium">Cửa hàng:</span> {order.storeName || 'N/A'}</p>
                      <p><span className="font-medium">Nhân viên:</span> {order.assignedEmployeeName || 'N/A'}</p>
                      <p><span className="font-medium">Tổng tiền:</span> {order.totalAmount?.toLocaleString('vi-VN')} VNĐ</p>
                      <p><span className="font-medium">Đã thanh toán:</span> {order.paidAmount?.toLocaleString('vi-VN')} VNĐ</p>
                    </div>
                  </div>
                </div>
                ...
                {order.stageHistory && order.stageHistory.length > 0 && (
                  <div className="mb-6">
                    <h4 className="font-semibold text-gray-900 mb-3">Lịch sử tiến độ</h4>
                          ...
                          <div key={index} className="flex items-center space-x-3 p-3 bg-gray-50 rounded-lg">
                            ...
                            <div className="flex-1">
                              ...
                              <p className="text-xs text-gray-500">
                                {new Date(stage.stageDate).toLocaleString('vi-VN')}
                              </p>
                            </div>
                            {stage.notes && (
                ...
                {order.orderDetails && order.orderDetails.length > 0 && (
                  <div>
                    <h4 className="font-semibold text-gray-900 mb-3">Chi tiết dịch vụ</h4>
                          ...
                          {order.orderDetails.map((detail, index) => (
                            <tr key={index}>
                              <td className="px-6 py-4 whitespace-nowrap text-sm font-medium text-gray-900">
                                {detail.decalServiceName || detail.decalTypeName}
                              </td>
